...
<div class="row">
	...
	@endif
	<div class="col-md-2">
		<button type="button" class="rounded-circle col-md-12 @if($is_mobile) col-xs-6 @endif btn btn-outline-info @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-draft"><i class="fas fa-edit"></i> @lang('sale.draft')</button>		@if(empty($pos_settings['disable_suspend']))
		<button type="button"
		class="rounded-circle col-md-12 @if($is_mobile) col-xs-6 @endif btn btn-outline-danger no-print pos-express-finalize"
	...
	<div class="col-md-3">
		<button type="button" class="rounded-circle col-md-12 btn btn-outline-warning @if($is_mobile) col-xs-6 @endif" id="pos-quotation"><i class="fas fa-edit"></i> @lang('lang_v1.quotation')</button>
	...
	<div class="col-md-3">
		...
		<button type="button" class="rounded-circle col-md-12 btn btn-outline-dark  @if(!$is_mobile) @endif no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif @if($is_mobile) col-xs-6 @endif" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt" aria-hidden="true"></i> @lang('lang_v1.checkout_multi_pay') </button>
	</div>
	<div class="col-md-2">
		<button type="button" class="rounded-circle col-md-12 btn btn-outline-success @if(!$is_mobile) @endif no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize @if($is_mobile) col-xs-6 @endif" data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-dollar-sign"></i> @lang('lang_v1.express_checkout_cash')</button>
	</div>
	<div class="col-md-2">

		@if(!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
		<button type="button" class="rounded-circle btn btn-outline-primary @if($is_mobile) col-xs-6 @endif" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions"> <i class="fas fa-clock"></i> @lang('lang_v1.recent_transactions')</button>
		@endif
	</div>
</div>
